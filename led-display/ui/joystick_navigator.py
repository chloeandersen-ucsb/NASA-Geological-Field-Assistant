...
class JoystickNavigator(QObject):
    ...
    def _highlight_btn(self, btn: QPushButton):
        ...
        self._scroll_to_btn(btn)

    def _scroll_to_btn(self, btn: QPushButton):
        """Auto-scroll any ancestor QScrollArea so btn is visible."""
        from PySide6.QtWidgets import QScrollArea
        parent = btn.parent()
        while parent is not None:
            if isinstance(parent, QScrollArea):
                parent.ensureWidgetVisible(btn, 20, 20)
                return
    ...
    def _buttons_on_page(self) -> list:
        ...
        for w in page.findChildren(QPushButton):
            if not (w.isVisible() and w.isEnabled()):
                continue
            # Skip buttons that live inside a QListWidget's item widgets
            ancestor = w.parent()
            in_list = False
