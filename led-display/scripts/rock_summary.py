...
DEFAULT_MODEL_PATH = PROJECT_ROOT / "led-display" / "models" / "tinyllama-1.1b-chat-v1.0.Q4_K_M.gguf"
...
def _load_llama() -> Llama:
    ...
    n_ctx = int(os.environ.get("SAGE_SUMMARIZER_CTX", "512"))
...
def _clean_generated_summary(text: str) -> str:
    # 1. Strip any weird markdown blocks the AI tries to add
    summary = re.sub(r"^```[a-zA-Z]*\n", "", text.strip())
    summary = re.sub(r"\n```$", "", summary)
    
    # 2. Ensure there's a clean line break before each raw text bullet
    summary = re.sub(r"(?<!\n)- ", "\n- ", summary)
    
    # 3. THE CHOP: Find the first actual category we asked for and delete everything above it
    match = re.search(r"(?m)^-\s*(Classification|Color & Appearance|Texture & Structure|Dimensions & Weight|Field Context):", summary, re.IGNORECASE)
    if match:
        summary = summary[match.start():]
        
    # 4. Clean up any lingering headers just in case
    summary = re.sub(r"(?i)^(summary|extracted field notes|report):\s*\n*", "", summary)
